...
6. Use the Write tool to write the composed message to that path. The content must be exactly the commit message with no trailing newline. Do NOT use a Bash heredoc — heredocs containing prose trip the Bash command classifier with `undefined node type: string` and force a manual permission prompt every run. For the same reason, never use `$(...)` command substitution inside a double-quoted string; assign to a variable first.
